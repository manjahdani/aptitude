...
public class CountingSkill extends Skill implements CounterObserverCapacity {
  private final HashMap<String, OrientedCountingLine> countingLines = new HashMap<String, OrientedCountingLine>();
  
  public void install() {
    OrientedCountingLine _orientedCountingLine = new OrientedCountingLine(1235, 700, 309, 664, 11, 1);
    this.countingLines.put("A", _orientedCountingLine);
    OrientedCountingLine _orientedCountingLine_1 = new OrientedCountingLine(1477, 324, 1676, 310, 11, (-1));
    this.countingLines.put("B", _orientedCountingLine_1);
  }
  
  @Pure
  public void Signal2Perception(final Object signal) {
    ArrayList<BBoxe2D> boundingBoxes = ((ArrayList<BBoxe2D>) signal);
    for (final BBoxe2D bb : boundingBoxes) {
      ...
      for (final String counting_line : _keySet) {
        if ((this.has_crossed_counting_line(bb.getBBOX(), this.countingLines.get(counting_line)) && 
          (!this.countingLines.get(counting_line).hasCrossed(bb.getGlobalID())))) {
          final int orientation = this.countingLines.get(counting_line).Orientation(bb.getDirection());
          if ((orientation != 0)) {
            this.countingLines.get(counting_line).incrementCount(bb.getClasseID());
          } else {
          }
        }
      }
  ...
  @Pure
  public boolean has_crossed_counting_line(final BBOX b, final OrientedCountingLine cl) {
    double _x = b.getX();
    double _y = b.getY();
  ...
  public HashMap<String, OrientedCountingLine> DisplayPerception(final int actionID) {
    if ((actionID == 1)) {
      ...
      for (final String counting_line : _keySet) {
        {
          int totalCount = 0;
          Collection<AtomicInteger> _values = this.countingLines.get(counting_line).counts.values();
          for (final AtomicInteger v : _values) {
            int _get = v.get();
            totalCount = (totalCount + _get);
          }
          Logging _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER = this.$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER();
          _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.info(((counting_line + " : ") + Integer.valueOf(totalCount)));
        }
      }
    }
